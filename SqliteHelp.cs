...
namespace LianjiaWebWorm
{
    ...
    class SqLiteHelper
    {
        ...
        /// <returns>The values.</returns>
        /// <param name="tableName">数据表名称</param>
        /// <param name="queryString">查找语句</param>
        public List<INFO> SelectValues(string queryString)
        {
            SQLiteDataReader sr =  ExecuteQuery(queryString);
            List<INFO> tables = new List<INFO>();
            while (sr.Read())
            {
                INFO h = new INFO();
                h.addr = sr.GetString(0);
                h.housetitle = sr.GetString(1);
                h.room = sr.GetString(2);
                h.area = sr.GetString(3);
                h.info = sr.GetString(4);
                h.price = sr.GetString(5);
                h.unitprice = sr.GetString(6);
                h.date = sr.GetString(7);
                tables.Add(h);
            }
            return tables;
        }
        ...
        {
            return dbConnection.GetSchema(table);
        }
        /// <summary>
        /// 更新指定数据表内的数据
        /// </summary>
